...
<script type="text/javascript">
	$(function() {
		...
		$('#ddd').click(function() {
			location.href = "<c:url value='/gogak/admin/personal/faqEdit.do?no=${vo.faqNo}'/>";
		});
		...
		$('#delete').click(function() {
			...
			if (str) {
				//true
				location.href = "<c:url value = '/gogak/admin/personal/faqDelete.do?no=${vo.faqNo}'/>";
			} else {
				event.preventDefault;
				location.href = "<c:url value='/gogak/admin/personal/faqDetail.do?no=${vo.faqNo}'/>";
			}
		});
	})
</script>
...
<main>
	...
	<div
		...
		<div style="margin: 5px; height: 95px; border: 1px solid lightgray;">
			<h2 style="padding-left: 50px; padding-top: 30px;">자주찾는 질문 상세보기(개인)</h2>
		</div>
		<!-- 본문 -->
		<div style="text-align: center; margin: 5px; border: 1px solid lightgray;">
			...
			<div id = "ab">
				<div>
					...
					<c:if test="${!empty afterVO.faqNo }">
						<a href="<c:url value='/gogak/admin/personal/faqDetail.do?no=${afterVO.faqNo}'/>" class = "font-color">
				...
				<div>
					...
					<c:if test="${!empty beforeVO.faqNo }">
						<a href="<c:url value='/gogak/admin/personal/faqDetail.do?no=${beforeVO.faqNo}'/>" class = "font-color">
